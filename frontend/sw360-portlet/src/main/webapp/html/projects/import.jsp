...
<script>
    ...
    function createDataSourceTable() {
        ...
        var checkedProjectId = '<%=PortalConstants.PROJECT_IMPORT_CHECKED_PROJECT%>${importable.externalIds.get(idName)}';
        result.push({
            ...
            content: function () {
                ...
                return $.ajax({
                    ...
                    data: {
                        ...
                        "<portlet:namespace/><%=PortalConstants.PROJECT_IMPORT_USER_ACTION__IMPORT%>":
                            '<%=PortalConstants.PROJECT_IMPORT_USER_ACTION__IMPORT_BDP%>'
                    }
                }).done(function (response) {
                    self.setTitle("Import result");
                    switch(response.<%=PortalConstants.PROJECT_IMPORT_RESPONSE__STATUS%>) {
                        case '<%=PortalConstants.PROJECT_IMPORT_RESPONSE__SUCCESS%>':
    ...
    function connectDBRequestSuccess(response, serverURL) {
        ...
        var responseCode = response.<%=PortalConstants.PROJECT_IMPORT_RESPONSE__STATUS%>;
        cleanMessages();

        switch(responseCode) {
            case '<%=PortalConstants.PROJECT_IMPORT_RESPONSE__DB_CHANGED%>':
                showLogin(serverURL);
                $.confirm({
                    ...
                    content: function () {
                        ...
                        return $.ajax({
                            ...
                            data: {
                                "<portlet:namespace/><%=PortalConstants.PROJECT_IMPORT_USER_ACTION__IMPORT%>":
                                    "<%=PortalConstants.PROJECT_IMPORT_USER_ACTION__UPDATEIMPORTABLES%>",
                                "<portlet:namespace/><%=PortalConstants.PROJECT_IMPORT_PROJECT_NAME%>":
                ...
                break;
            case '<%=PortalConstants.PROJECT_IMPORT_RESPONSE__DB_CONNECT_ERROR%>':
                flashErrorMessage('Could not connect to DB.');
                break;
            case '<%=PortalConstants.PROJECT_IMPORT_RESPONSE__DB_URL_NOT_SET%>':
                flashErrorMessage('Please enter a server URL');
                break;
            case '<%=PortalConstants.PROJECT_IMPORT_RESPONSE__UNAUTHORIZED%>':
    ...
    function importUpdateProjectTable(response) {
        var importables = response.<%=PortalConstants.PROJECT_IMPORT_RESPONSE__NEW_IMPORTABLES%>;
        var projectList = [];

        importables.forEach(function(el) {
            el = JSON.parse(el);
            var checkedProjectId = '<%=PortalConstants.PROJECT_IMPORT_CHECKED_PROJECT%>' + el.externalId;
            projectList.push({
    ...
    function updateDataSource() {
        ...
        var serverUrl = $("#input-dataserver-url").val(),
            data = new Object();
        data["<portlet:namespace/><%=PortalConstants.PROJECT_IMPORT_USER_ACTION__IMPORT%>"] = "<%=PortalConstants.PROJECT_IMPORT_USER_ACTION__NEWIMPORTSOURCE%>";
        data["<portlet:namespace/><%=PortalConstants.PROJECT_IMPORT_SERVER_URL%>"] = serverUrl;
        data["<portlet:namespace/><%=PortalConstants.PROJECT_IMPORT_USERNAME%>"] = $("#input-dataserver-user").val();
        data["<portlet:namespace/><%=PortalConstants.PROJECT_IMPORT_PASSWORD%>"] = $("#input-dataserver-pw").val();
        data["<portlet:namespace/><%=PortalConstants.PROJECT_IMPORT_PROJECT_NAME%>"] = $("#input-project-name").val();
    ...
    function disconnectDataSource() {
        var data = new Object();
        data["<portlet:namespace/><%=PortalConstants.PROJECT_IMPORT_USER_ACTION__IMPORT%>"] = "<%=PortalConstants.PROJECT_IMPORT_USER_ACTION__DISCONNECT%>";
        $.ajax({
            url: '<%=ajaxURL%>',
            type: 'POST',
            data: data,
